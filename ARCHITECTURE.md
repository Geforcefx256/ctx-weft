...
**操作账本（spec: tool-operations；协议住在 `protocols/capability.py`——账本是能力调用契约的持久化半边，不是独立的域）**：gateway 是所有工具调用（含 silent/dispatch）的单一咽喉，按五步序记账——持久身份 → 授权 → prepared → CAS started → provider → completed（持久确认）→ 幂等 TOOL_RESULT → CapabilityFinished。`operation_id` **就是该次调用在摄入点铸造的内部 tool_call 标识**（`tc_...`，见 capability `conversation-integrity`）——随 assistant 回合一同落库，恢复时由 gateway 从 `tool_call_id` **读出**而非重算；调用侧不再铸第二条身份，也不再经共享的 `provider_ctx` 转移所有权（那个可变字段忘了清会泄漏到后续无关 invoke）。同参两次合法调用因回合锚/序号不同而得到不同 id。裸 wire id（`is_internal_call_id` 判假）→ 账本全程旁路。账本已 completed → 短路回放结果不再打 provider（同逻辑调用重入）；HITL park → `waiting_human`。`OperationStore`（内存默认 / SQL）写失败 → PersistenceUnavailableError 隔离。**恢复策略（spec: tool-operations，reliability-wp6）**：reconcile 完成判定按**逻辑身份**（账本 COMPLETED / 确定性 memory_result_id 双通道，不再看 wire id——call_1 复用不串扰）；未完成的按「状态 × recovery_policy」分派。**策略只有两值**（`idempotent` / `reviewed`，默认 `reviewed`；非法取值在 resolver 启动校验响亮失败，不静默降级）：`idempotent` 同 tool_call_id 重跑（控制工具即此类——core 自有的同身份幂等状态迁移，靠**声明**而非 gateway 里的名字前缀特判）；`reviewed` 时 core 绝不自行决定，问**重跑授权** `RerunAuthorizer.authorize_rerun`。它与事前授权 `Authorizer` 是两个问题、两条注册通道（`register_capability(rerun_authorizer=…)` / `set_capability_rerun_authorizer(key, …)`，解析同为三级 capability_id > provider 前缀 > 无），**由宿主注册而非 provider 实现**——知道怎么查证外部真值的对象未必是提供工具的对象，绑死在 provider 上则 MCP 工具永远挂不上。绝不复用 `authorize` 回答重跑：默认 `AllowAllAuthorizer` 会答 allowed=True，等于自动重跑副作用。返回 `AuthorizationDecision`：`allowed=True` → 同 tool_call_id 重跑（其后仍走一次事前授权，「当初准跑」≠「现在还准跑」）；`allowed=False` → `message` 成为这次调用的工具结果、账本作结（`result_is_error` 按内容定）；`needs_human` → 既有 HITL park（账本落 `waiting_human`，决定缓存用独立的 rerun stage），被删掉的 `resolve_operation` 三个动作由此还原。未注册 → core 代为作结「无从查证」。`prepared` 之前（**持久**账本查不到记录）判为确定未启动、首执安全——判据必须是 `OperationStore.durable`，内存账本重启后一片空白，「没跑过」与「跑过但记录没了」不可区分（真子进程强退的 O-T05 正踩在这里）；账本不持久时 runtime 启动告警。**「不确定」是一种工具结果而非控制流**：不停机、不设专属错误码/事件/处置 API，agent 下一轮读到结果自行决定——原先的 `TOOL_OUTCOME_UNKNOWN` / `OperationUncertain` / `resolve_operation` / 恢复闸门 / `OperationStatus.UNKNOWN` 全部删除。重复调用防护归 Provider 自理。**作结 = 账本 CAS completed + 把 result 经收敛写成 TOOL_RESULT，然后照常续跑**——不改 task 状态、不发专属事件。控制工具（core 幂等状态迁移）与冷 HITL 决定在案（prepared-未-started 语义）走首执。`OperationRecord` 的恢复判据只有 `status` / `result` / `attempts` / `effective_args`（最后一项是授权与 HITL 改写之后交给 provider 的真实入参，**未脱敏**，供重跑授权去外部查证；不这样存，任何改过参的调用它都查不对）；`args_hash` / `memory_result_id` / `task_id` 是审计随行记录，恢复判定不读。账本上**不存** `recovery_policy`——策略读 capability 的活声明，冗余一份只会在工具改判后按过期策略走。
...
- **执行时长/轮数限制**：core **不做**。任务级 deadline、actor 轮数上限、provider 超时都是宿主策略——宿主自己计时并调 `cancel_task` / `cancel_agent`，provider 级超时由 provider 的工具实现自理。三个从未被执行的旧字段（`max_turns_per_agent` / `timeout_per_step_sec` / `Task.timeout_ms`）已删除（声明了限制却无运行时消费者）。`max_turns_per_act` / `max_turns_per_observe` / `context_limit` / LLM 自愈预算等既有的**真正生效**的上限不受影响。
