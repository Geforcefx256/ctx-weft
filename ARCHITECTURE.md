...
**操作账本（spec: tool-operations，change reliability-wp5）**：gateway 是所有工具调用（含 silent/dispatch）的单一咽喉，按五步序记账——持久身份 → 授权 → prepared → CAS started → provider → completed（持久确认）→ 幂等 TOOL_RESULT → CapabilityFinished。`operation_id` **就是该次调用在摄入点铸造的内部 tool_call 标识**（`tc_...`，见 capability `conversation-integrity`）——随 assistant 回合一同落库，恢复时由 gateway 从 `tool_call_id` **读出**而非重算；调用侧不再铸第二条身份，也不再经共享的 `provider_ctx` 转移所有权（那个可变字段忘了清会泄漏到后续无关 invoke）。同参两次合法调用因回合锚/序号不同而得到不同 id。裸 wire id（`is_internal_call_id` 判假）→ 账本全程旁路。账本已 completed → 短路回放结果不再打 provider（同逻辑调用重入）；HITL park → `waiting_human`。`OperationStore`（内存默认 / SQL）写失败 → PersistenceUnavailableError 隔离。**恢复策略（spec: tool-operations，reliability-wp6）**：reconcile 完成判定按**逻辑身份**（账本 COMPLETED / 确定性 memory_result_id 双通道，不再看 wire id——call_1 复用不串扰）；未完成的按「状态 × recovery_policy」分派。**策略只有两值**（`idempotent` / `reviewed`，默认 `reviewed`；非法取值在 resolver 启动校验响亮失败，不静默降级）：`idempotent` 同 op_id 重跑；`reviewed` 时 core 绝不自行决定，问 Provider 的 `OperationAdjudicator`（靠 `isinstance` **发现**，不靠 capability 声明，因此不存在「声明了却没实现」的失败模式，原先 registry + resolver 两层校验一并删除）。裁决者只回答**该不该重跑**：`rerun=True` → 同 op_id 重跑；`rerun=False` → 它给的 `result` 成为这次调用的工具结果、账本作结。未实现裁决接口 → core 代为作结「无从查证」。**「不确定」是一种工具结果而非控制流**：不停机、不设专属错误码/事件/处置 API，agent 下一轮读到结果自行决定——原先的 `TOOL_OUTCOME_UNKNOWN` / `OperationUncertain` / `resolve_operation` / 恢复闸门 / `OperationStatus.UNKNOWN` 全部删除。重复调用防护归 Provider 自理。**作结 = 账本 CAS completed + 把 result 经收敛写成 TOOL_RESULT，然后照常续跑**——不改 task 状态、不发专属事件。控制工具（core 幂等状态迁移）与冷 HITL 决定在案（prepared-未-started 语义）走首执。`OperationRecord` 上除 `status` / `result` / `attempts` 外的字段（`recovery_policy` / `args_hash` / `memory_result_id` / `task_id`）是审计随行记录，恢复判定不读——策略取自 capability 的活声明。
